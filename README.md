...
tweaking), and requires a locally (or remotely) running `groestlcoind` (version 2.16.0 or
above) that is fully caught up with the network you're testing on.
Pruning (prune=n option in groestlcoin.conf) is not currently supported.
